...
export default function Editor() {
  ...
  const handleRestoreVersion = async (version) => {
    if (!confirm(`Restore to "${version.label || `v${version.versionNumber}`}"? Current content will be saved first.`)) return;
    try {
      const { data } = await api.post(
        `/documents/${docId}/versions/restore/${version._id}`
      );
      if (data.content?.ops) {
    ...
    <div style={styles.app}>
      ...
      <div style={styles.topbar}>
        <div style={styles.topLeft}>
          ...
          <span style={{
            fontSize:'12px',
            color: saveStatus === 'Saved' ? 'var(--success)' : 'var(--text3)',
            whiteSpace:'nowrap',
            flexShrink:0,
          }}>
            {saveStatus}
